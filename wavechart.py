...
# Chart type selection including 'Wave Chart'
CHART_TYPE = st.selectbox("Select Chart Type:", 
                          ["Line Chart", "Candlestick Chart", "OHLC Chart", "Scatter Plot", "Area Chart", "Bar Chart", "Histogram", "Heatmap", "Wave Chart"], 
                          index=0)
...
def plot_stock_data(data, price_types, chart_type):
    ...
    if chart_type == "Wave Chart":
        # Create a wave chart for selected price types
        fig = go.Figure()
        ...
        for price_type in price_types:
            # Apply sine wave transformation to the selected price type
            price_column = df[price_type].astype(float)
            sine_wave = np.sin(np.linspace(0, 2 * np.pi, len(df))) * price_column

            # Add the sine wave plot for the current price type
            fig.add_trace(go.Scatter(
                x=df['datetime'],
                y=sine_wave,
                mode='lines',
                name=f'{price_type.capitalize()} Wave',
                line=dict(color=price_colors.get(price_type, "gray"), width=2)
            ))

        # Update the layout for the Wave Chart
        fig.update_layout(
            title=f"Wave Chart for {SYMBOL}",
            xaxis_title="Date",
            yaxis_title="Wave-Transformed Price",
